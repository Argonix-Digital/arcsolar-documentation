...
- **Documented QA flags are unwired.** Four flag names exist in `.env.example` and the contract but are read by no application code. Runtime behaviour is controlled by `ROOF_DESIGN_ORIGIN` and `ROOF_DESIGN_EXCHANGE_KEY` instead. Whether the flags were meant to be wired or removed is unresolved.
...
- **No entitlement gate on the new path.** A `roof_vision` entitlement exists but is applied to the older analysis paths, not to this route or panel.
...
- **Availability.** Availability is not controlled by a feature flag you can read. Confirm against the deployment's actual configuration. Issuing through the ordinary launch path was still blocked by missing valid QA payment configuration at the time of writing.
</Visibility>
...
- [Get help from ArcSolar](/reference/support) — how to ask whether a gated feature is enabled for you.
