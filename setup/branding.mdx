...
If Google Fonts search returns nothing, either the search genuinely has no match, or the feature is not configured on your deployment — in that case ArcSolar reports the deployment's own message rather than pretending there were no results. You can still upload a **licensed font file** through the assets control below the font fields.
...
<Visibility for="agents">
  Only the workspace owner can render the preview, because assembling it uses the same authority that approves a version. A non-owner sees the reason the call gave rather than a blank frame, and the button stays available so the reason is discoverable. Footer reads `Draft version N, not approved.`
</Visibility>
