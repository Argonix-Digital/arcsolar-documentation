...
<Frame caption="Standalone design build with a synthetic roof image and demonstration panels; in the deployed editor the roof image is Google satellite imagery. The Model catalogue is open over the canvas, with the tool rail at the left and the design downloads at the right.">
  <img src="/images/roof-design-editor-workspace.png" alt="Roof Design editor showing nine demonstration panels on a synthetic roof image, the Model catalogue open over the canvas, the ten-tool rail at the left and the six design download buttons at the right." />
</Frame>
...
<Visibility for="humans">
  ...
  **Imagery and tools.** **Images and plans** calibrates a floor plan or photo you supply against a known distance and a north direction. **Solar production** overlays Google Solar production and shade.
  ...
  <Frame caption="The status line under the canvas reports the roof check: the module size and wattage, the projected height and width gaps, and `1 estimated faces · pitch unmeasured`. Faces read from imagery are estimates, and pitch is never derived from an image.">
    <img src="/images/roof-design-roof-analysis.png" alt="Roof image with demonstration panels placed on it and the canvas status line reading 1762 × 1134 mm · 450 W · 20 × 30 mm gaps · 1 estimated faces · pitch unmeasured." />
  </Frame>

  <Frame caption="Hovering beside an existing panel — the one badged `E` — reveals one adjoining footprint marked `+`. The extension inherits the neighbour's angle, tilt and product so the array stays consistent. The edit bar above carries the selected model and the live panel counts.">
    <img src="/images/roof-design-adjacency-preview.png" alt="Design canvas in Build mode with an adjoining panel footprint marked with a plus sign beside an existing panel badged E, the edit bar above showing the selected model and the live panel counts, and the Build-mode help line under the canvas." />
  </Frame>
...
<Visibility for="agents">
  ...
  **Per-mode help text, verbatim.** Rendered under the canvas.
  ...
  - **Build** places one panel per click, or drags a rectangular array along the current azimuth. Hovering beside an existing panel reveals one dashed adjoining footprint with `+`; the extension inherits the neighbour's angle, tilt and product so arrays stay consistent.
